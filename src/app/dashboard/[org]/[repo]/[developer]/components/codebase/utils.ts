export const truncateString = (string = "", length = 20, zoom = 1): string => {
  const adjustedLength = Math.floor(length * zoom);
  return string.length > adjustedLength + 3
    ? string.substring(0, adjustedLength) + "..."
    : string;
};
