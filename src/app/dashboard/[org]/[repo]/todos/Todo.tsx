...
const Todo: React.FC<TodoProps> = ({ org, repo }) => {
  ...
  const [generatingResearch, setGeneratingResearch] = useState<
  ...
  const handleGenerateResearch = (todoId: number) => {
    setGeneratingResearch((prev) => ({ ...prev, [todoId]: true }));
    // Simulating research generation
    setTimeout(() => {
      setGeneratingResearch((prev) => ({ ...prev, [todoId]: false }));
    }, 3000);
  };
