...
export async function fetchNewJiraIssues({
  ...
  try {
    ...
    for (const issue of issues) {
      ...
      // Evaluate the Jira issue quality
      ...
      if (evaluation.evaluationScore >= 4) {
        ...
        console.log(
          `Repo ${project.repoFullName}: Jira issue ${issue.id} evaluation score too low (${evaluation.evaluationScore}), skipping GitHub issue creation`,
        );
      }
    }
  } catch (error) {
