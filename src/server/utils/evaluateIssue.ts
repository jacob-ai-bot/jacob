...
export async function evaluateIssue({
  ...
  const userPrompt = `Based on the following information:
    ...
    - Specific risk areas. Don't overcomplicate, keep it concise and focused on risks that are common in a medium-sized codebase with a small team. Don't mention every single risk, just say so.
...
const EvaluationJiraIssueSchema = z.object({
  evaluationScore: z.number().min(1).max(5),
  feedback: z.string().optional(),
});

export type EvaluationJiraIssue = z.infer<typeof EvaluationJiraIssueSchema>;
...
export async function evaluateJiraIssue({
  ...
  baseEventData?: BaseEventData;
}): Promise<EvaluationJiraIssue> {
  const systemPrompt = `You are an expert software engineer and technical evaluator. Your task is to evaluate the following Jira issue for its suitability for AI coding agent completion.

Provide an evaluation score between 1 and 5 (half-points acceptable), where:

- 1: The issue lacks sufficient detail for any developer to begin work.
- 2: The issue has minimal detail but is still too vague for effective action.
- 3: The issue provides some detail but lacks key information.
- 4: The issue is well-defined with most necessary information.
- 5: The issue is thoroughly defined with clear instructions and requirements.

If the score is less than 4, provide a one-sentence feedback message informing the user what needs to be changed to make the ticket actionable by the AI agent. If the score is 4 or higher, the feedback should be an empty string.`;
